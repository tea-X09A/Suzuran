...
# シグナル
signal debug_state_changed(is_open: bool)  # デバッグメニューの開閉状態が変化した時
signal debug_value_changed(key: String, value: Variant)  # デバッグ値が変更された時

# UI要素
var debug_menu: CanvasLayer = null  # デバッグメニュー全体のコンテナ
var menu_container: VBoxContainer = null  # メニュー項目を縦に並べるコンテナ
var debug_items: Array[Dictionary] = []  # 登録されているデバッグ項目のリスト
var current_selection: int = 0  # 現在選択中の項目インデックス
var selection_panel: Panel = null  # 選択中の項目を強調表示するパネル（再利用）

# 状態管理
var is_open: bool = false  # デバッグメニューが開いているか
var debug_values: Dictionary = {}  # デバッグ値の保存用辞書
